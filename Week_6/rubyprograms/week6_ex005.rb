...
As a deterrent against cheating, when the game loads a save file it performs 
a simple check against the file's modification time. If it differs from the 
...
file to the time the filesystem has associated with the file. 
...
class Game_Util
  ...
  def load_game(file)
    lines = File.new(file, "r+b").readlines  
    score = lines[0].gsub!(/\n/,"" )
    time_from_file = lines[1]
    
    raise "I suspect you of cheating." if File.mtime(file) != Time.at(time_from_file.to_i)
...
util.save_game("game.sav")
sleep(2)
util.load_game("game.sav") # => "Your saved score is 1000."
# Now let's cheat by increasing our score to 9000
open("game.sav", "r+b") { |f| f.write("9") }
util.load_game("game.sav") # RuntimeError: I suspect you of cheating.
